...
class BaseTrainer():
    def __init__(self, rank, cfg):
        ...
        # === UVT modification (CPU-tolerance): 无 CUDA 时跳过绑卡（开发机冒烟/测试用）===
        if torch.cuda.is_available():
            torch.cuda.set_device(local_rank)
        # === UVT modification end ===
        # === UVT modification (CPU-tolerance): 无 CUDA 时落 CPU ===
    ...
    def run(self):
        ...
        if os.path.exists(resume_ckt_path):
            ...
            if 'rng_states_per_rank' in latest_ckt and self.rank in latest_ckt['rng_states_per_rank']:
                ...
                if torch.cuda.is_available() and local_rng_state_dict['torch_cuda_rng_state'] is not None:
    ...
    def configure_scalers(self, sd=None, load_sd=False):
        scaler = torch.cuda.amp.GradScaler(enabled=self.use_amp and torch.cuda.is_available() and self.amp_dtype == torch.float16)
