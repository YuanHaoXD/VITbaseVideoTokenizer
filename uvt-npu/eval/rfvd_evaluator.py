...
# === NPU: triton shim 必须在 import torch 之前（见 utils/accel.py 说明）===
try:
...
from utils import accel  # === NPU: 设备统一抽象（npu/cuda/cpu），替代散落的 .cuda() ===
...
class UCFrFVDEvaluator:
    def __init__(
        ...
        self.model = model.to(accel.device()).eval()
        ...
        self.perceptual_loss = lpips.LPIPS(net='vgg').to(accel.device()).eval()
    ...
    def evaluate(self, no_fvd=False):
        ...
        with torch.inference_mode():
            for batch_idx, batch in enumerate(tqdm(self.loader)):
                vb = batch['gt'].to(accel.device())
                n_frames = vb.size(2)
                if self.repeat_to_16:
                    vb = self.repeat_to_16_frames(vb)

                with accel.autocast(enabled=self.use_amp, dtype=self.amp_dtype):
