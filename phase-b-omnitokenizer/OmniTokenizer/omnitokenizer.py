...
from .modules.temporal_fold import TemporalFold2x, TemporalUnfold2x  # Phase B-2 learned 臂
...
class VQGAN(pl.LightningModule):
    def __init__(self, args):
        ...
        # Phase B-1 / B-2 (02 §3.1 / §3.2): 三模式 + 三臂统一开关
        if not hasattr(args, "temporal_attn_mode"):
            args.temporal_attn_mode = "causal"
        if not hasattr(args, "temporal_fold_mode"):
            args.temporal_fold_mode = "single"
        assert args.temporal_fold_mode in ("single", "learned", "avgpool"), \
            f"temporal_fold_mode must be single/learned/avgpool, got {args.temporal_fold_mode}"
        # avgpool 臂复用现成的 defer_temporal_pool 机制 (omnitokenizer.py :792-795)，零额外改动
        if args.temporal_fold_mode == "avgpool":
            args.defer_temporal_pool = True
        self.temporal_attn_mode = args.temporal_attn_mode
        self.temporal_fold_mode = args.temporal_fold_mode
        ...
        self.encoder = OmniTokenizer_Encoder(
            ...
            temporal_attn_mode=args.temporal_attn_mode, temporal_fold_mode=args.temporal_fold_mode,
        )
    ...
    def add_model_specific_args(parent_parser):
        ...
        # Phase B-1: 时间注意力三模式 (02 §3.1)；Phase B-2: 层级时间 patchify 三臂 (02 §3.2)
        parser.add_argument('--temporal_attn_mode', type=str, default='causal',
                            choices=['tubelet', 'causal', 'full'])
        parser.add_argument('--temporal_fold_mode', type=str, default='single',
                            choices=['single', 'learned', 'avgpool'])
...
class OmniTokenizer_Encoder(nn.Module):
    def __init__(self, image_size, patch_embed, norm_type, block='tttt', window_size=4, spatial_pos="rel",
                    ...
                    temporal_attn_mode="causal", temporal_fold_mode="single"):
        super().__init__()
        ...
        self.block = block
        # Phase B-2: learned 臂在空间/时间 transformer 之间插 TemporalFold2x (02 §3.2)
        self.temporal_fold_mode = temporal_fold_mode
        self.temporal_fold = TemporalFold2x(dim) if temporal_fold_mode == "learned" else None
        ...
        # Phase B-1 (02 §3.1): 仅时间 transformer 透传 temporal_attn_mode（空间 transformer 保持默认 causal，但 is_spatial=True 时忽略它）
        transformer_kwargs["temporal_attn_mode"] = temporal_attn_mode
    ...
    def encode(
        ...
        # Phase B-2 learned 臂 (02 §3.2)：空间与时间 transformer 之间插 TemporalFold2x，
        # 2× 学习式层级折叠（patchify 已做 2×，Fold 再做 2× = 4×，对齐 single 臂 pt=4）。
        # 锚帧（位 0）由 Fold 内部隔离（tokens[:,1:] 才参与），与 :910-914 的 pool 隔离一致。
        if self.temporal_fold is not None:
            tokens = rearrange(tokens, 'b t h w d -> b t (h w) d')
            tokens = self.temporal_fold(tokens)
            tokens = rearrange(tokens, 'b t (h w) d -> b t h w d', h=new_h, w=new_w)
...
class OmniTokenizer_Decoder(nn.Module):
    def __init__(self, image_size, patch_embed, norm_type, block='tttt', window_size=4, spatial_pos="rel",
                    ...
                    sequence_length=17, temporal_attn_mode="causal", temporal_fold_mode="single"):
        super().__init__()
        self.gen_upscale = gen_upscale
        if gen_upscale is not None:
            patch_size *= gen_upscale

        # Phase B-2: learned 臂的 Unfold（encoder Fold 的镜像，dim 与 Fold 一致）
        self.temporal_fold_mode = temporal_fold_mode
        self.temporal_unfold = TemporalUnfold2x(dim) if temporal_fold_mode == "learned" else None
        ...
        # Phase B-1 (02 §3.1): 仅时间 transformer 透传 temporal_attn_mode
        transformer_kwargs["temporal_attn_mode"] = temporal_attn_mode
    ...
    def decode(
        ...
        # Phase B-2 learned 臂 (02 §3.2)：encoder Fold 的镜像——时间 transformer 之后、空间 transformer 之前插 Unfold2x。
        # 锚帧隔离同 Fold（tokens[:,1:] 参与）。
        if self.temporal_unfold is not None:
            tokens = rearrange(tokens, '(b h w) t d -> b t (h w) d', b=b, h=h, w=w)
            tokens = self.temporal_unfold(tokens)
            tokens = rearrange(tokens, 'b t (h w) d -> (b h w) t d', h=h, w=w)
