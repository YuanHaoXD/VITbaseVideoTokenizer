...
class Attention(nn.Module):
    def __init__(
        ...
        scale=8,
        spatial_pos="rel",
        temporal_attn_mode="causal",  # Phase B-1: {tubelet, causal, full} 时间注意力三模式 (02 §3.1)
    ):
        super().__init__()
        self.heads = heads
        self.causal = causal
        self.scale = scale
        # Phase B-1: 仅在时间注意力路径 (is_spatial=False) 生效；空间路径忽略此参数
        assert temporal_attn_mode in ("tubelet", "causal", "full"), \
            f"temporal_attn_mode must be one of tubelet/causal/full, got {temporal_attn_mode}"
        self.temporal_attn_mode = temporal_attn_mode
    ...
    def forward(
        ...
        if hasattr(F, "scaled_dot_product_attention") and torch.__version__ >= "2.1.0":
            ...
            if not is_spatial:
                # Phase B-1 (02 §3.1): 时间注意力三模式统一走 SDPA 路径——纪律要求三模式
                # 不得混入非 SDPA 手写路径（官方 README 承认两路径数值不等价）。
                # 序列此时即时间轴 ((b h w) t d)，N=时间位数。
                if self.temporal_attn_mode == "tubelet":
                    T1 = N
                    idx = torch.arange(T1, device=q.device)
                    # 位 t 只见 {t-1, t}：对角线 + 下对角线为 True（SDPA bool mask: True=允许）
                ...
                # spatial: 原有 SDPA 逻辑（rel pos bias + mask + causal）
                if exists(mask):
                    mask = F.pad(mask, (self.num_null_kv, 0), value=True)
                    mask = rearrange(mask, 'b j -> b 1 1 j')

                if self.spatial_pos == "rel":
                    h, w = int(math.sqrt(N)), int(math.sqrt(N))
...
class Transformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        window_size=4,
        spatial_pos="rel",
        temporal_attn_mode="causal",  # Phase B-1: 透传给 't' block 的 Attention (02 §3.1)
    ):
        super().__init__()
