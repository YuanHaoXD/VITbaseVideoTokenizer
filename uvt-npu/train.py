...
# === NPU: triton 兼容 shim —— 必须在 import torch 之前！===========================
# 本机 triton 3.6 改名了 AttrsDescriptor，而 import torch 会自动加载 torch_npu→inductor
# → from triton...import AttrsDescriptor → ImportError 崩。此处先补回该属性，再 import torch。
# （与 utils/accel.py 里的 shim 等价，这里内联是为了保证 import torch 之前的绝对顺序。）
try:
...
import trainers
import utils
from utils import accel  # === NPU modification: 导入即完成 triton shim + import torch_npu（注册 NPU 后端）===
...
def main_worker(rank, cfg):
    ...
    if manualSeed != -1:
        ...
        if accel.is_available():
            accel.manual_seed_all(manualSeed)
...
def main():
    ...
    if world_size > 1:
        # torchrun 分布式路径：先绑卡再建组（HCCL/NCCL 要求每进程独占一卡）
