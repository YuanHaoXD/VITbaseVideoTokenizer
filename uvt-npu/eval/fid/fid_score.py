...
# [UVT vendored] 唯一改动：原 `from inception import InceptionV3` 改为包内相对导入
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    # 前向兼容：scipy 老版 sqrtm(..., disp=False) 返回 (result, est_error) 元组；
    # 新版移除了 disp kwarg 且直接返回数组。统一兼容两种 API（vendored pytorch-fid 修正）。
    import inspect as _inspect
    if "disp" in _inspect.signature(linalg.sqrtm).parameters:
