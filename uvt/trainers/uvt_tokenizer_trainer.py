...
class UVTTokenizerTrainer(BaseTrainer):
    ...
    def configure_scalers(self, sd=None, load_sd=False):
        ...
        g_scaler = torch.cuda.amp.GradScaler(enabled=enabled)
        d_scaler = torch.cuda.amp.GradScaler(enabled=enabled)
    ...
    def _iter_step(self, data, is_train):
        ...
        with torch.autocast(device_type=('cuda' if torch.cuda.is_available() else 'cpu'), dtype=self.amp_dtype, enabled=self.use_amp):
