...
# === NPU modification: decord 在 aarch64 无轮子 → 惰性导入。null128 假数据路径不触发；
#     真实视频解码（VideoReader）才导入并设 torch bridge（仅首次）。真实训练若需视频解码，
#     可在此换 opencv/pyav 后端（待真实数据阶段处理）。===
def _decord():
    import decord
    if not getattr(decord, "_uvt_bridge_set", False):
        decord.bridge.set_bridge('torch')
        decord._uvt_bridge_set = True
    return decord
...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # === UVT modification (D-1): 联合训练需要模态标记 ===
        # is_video=True 供 losses/distill.py 屏蔽纯图像 batch 的视频蒸馏项（05 篇 D-1 卡）。
        # 双键返回（v1.3 契约修订①）：'video' 为规范键，'gt' 为 LARP 兼容别名，
        # 与 D-2 image_dataset 对称——任何读规范键 'video' 的下游（如 estimate_latent_stats）
        # 在图像/视频 batch 上行为一致，不再只对图像可用。
        return {'video': video_data, 'gt': video_data, 'path': path, 'label': label, 'is_video': True}
